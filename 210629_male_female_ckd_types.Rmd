...
cha_kn <-read.csv("~/proj_ua/data/ua_full_10types_new.v7.csv", fileEncoding = 'utf-8')
...
cha_kn2 <- cha_kn %>%
  mutate(
    ...
    ckd2 = ifelse(eGFR_ab==1&proteinuria==1,"proteinuric CKD",
                  ifelse(eGFR_ab==1&proteinuria!=1, "non-proteinuric CKD",
                         ifelse(eGFR_ab==0&proteinuria==1, "isolated proteinuric CKD", "normal"))),
...
glimpse(df_summary45)


table(df_summary$ckd)
```

```{r eGFR60}
df_sub <- df_summary %>%
  filter(ckd2==c("isolated proteinuric CKD", "proteinuric CKD", "non-proteinuric CKD"))
...
```
```{r eGFR45}
df_sub45 <- df_summary45 %>%
  filter(eGFR_ab45==1,
         #ckd2==c("isolated proteinuric CKD", 
          #       "proteinuric CKD", 
           #      "non-proteinuric CKD")
         )

sex45 <- ggplot(df_sub45) +
  geom_bar(aes(x=eGFR_ab45, fill=male)
           , position="fill", width = 0.5
           ) +
  #scale_fill_manual(values=c("gray1","gray30", "gray55", "gray80")) +
  xlab("Sex") +
...
stat_tbl1_60 <- tableby(ckd2~ male , data=df_sub, total=FALSE)
summary(stat_tbl1_60, text=TRUE)
kable(summary(stat_tbl1_60))
```
```{r stat_tbl1_45}
#isolated proteinuria
stat_tbl2 <- tableby(ckd2~ male , data=df_sub45, total=FALSE)
summary(stat_tbl2, text=TRUE)
...
```
```{r stat_tbl2_45}
stat_tbl2_45 <- tableby(male~ ckd + ckd2 , data=df_sub45, total=FALSE)
summary(stat_tbl2_45, text=TRUE)
kable(summary(stat_tbl2_45))
```
