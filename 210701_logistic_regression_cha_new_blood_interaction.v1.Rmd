...
glm45_non_t <- glm(eGFR_ab45 ~ . + he_ubld*male , family=binomial, data=df45_non)
summary(glm45_non_t)
...
```
```{r 45model glm proteinuria, test}
glm45_pro <- glm(eGFR_ab45 ~ male + age + he_uph + he_unitr + he_usg + 
                   #he_upro + 
                   he_uglu + he_uket + he_ubil + he_ubld + he_uro + leucocyte, family=binomial, data=df45_pro)
summary(glm45_pro)
...
glimpse(df45_pro)
table(df45_pro$eGFR_ab45)
df45_pro_g <- df45_pro %>%
  mutate(
    eGFR_ab45 = as.character(eGFR_ab45)
  )

bld45_g <- ggplot(df45_pro_g) +
  geom_bar(aes(x=he_ubld, fill=eGFR_ab45)
           , position="fill", width = 0.5
           ) +
  scale_fill_manual(values=c("gray1","gray30", "gray55", "gray80")) +
...
```{r correlation blood and ??}
corr <- df45_pro[,c(10,1:9,11:13)]
glimpse(corr)
