---
title: "comparison p values using manual code vs DeLong method and booststrap from pRCO package"
...
df <- read.csv("~/proj_ua/data/df_pvalue.csv")
df45 <- read.csv("~/proj_ua/data/df_pvalue45.csv")
glimpse(df)
glimpse(df45)
...
## internal non-protein vs external non-protein
roc_in_en_delong <- roc.test(roc_in, roc_en)
roc_in_en <- roc.test(roc_in, roc_en, method="bootstrap", boot.n=10000)
print(roc_in_en_delong);roc_in_en

## internal protein vs external protein
roc_ip_ep_delong <- roc.test(roc_ip, roc_ep)
roc_ip_ep <- roc.test(roc_ip, roc_ep, method="bootstrap", boot.n=10000)
print(roc_ip_ep_delong);roc_ip_ep
```
...
## internal non-protein vs external non-protein
roc45_in_en_delong <- roc.test(roc45_in, roc45_en)
roc45_in_en <- roc.test(roc45_in, roc45_en, method="bootstrap", boot.n=10000)
print(roc45_in_en_delong);roc45_in_en

## internal protein vs external protein
roc45_ip_ep_delong <- roc.test(roc45_ip, roc45_ep)
roc45_ip_ep <- roc.test(roc45_ip, roc45_ep, method="bootstrap", boot.n=10000)
print(roc45_ip_ep_delong);roc45_ip_ep
```
