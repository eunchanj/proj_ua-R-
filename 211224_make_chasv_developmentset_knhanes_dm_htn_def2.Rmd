...
kn_sel <- kn %>%
  select(c(ID, HE_DM, HE_HP, DE1_pt, DI1_pt)) %>%
  rename(
    id = "ID",
  ) %>%
  mutate(
    dm = ifelse(HE_DM==3&DE1_pt==1,1,0),
    htn = ifelse(HE_HP==3&DI1_pt==1,1,0)
  ) %>%
  select(c(id,dm,htn))
...
```{r, write dataset}
#write.csv(df_new, "~/proj_ua/data/chasv_development.v2.2.csv", fileEncoding = "utf-8", row.names = F)
```
